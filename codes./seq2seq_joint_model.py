...
import math
'''
This approach is motivated by the work in "Solving Aspect Category Sentiment Analysis as a Text Generation Task"
'''
import os
import random
import warnings
...
from test_joint import predict_val, predict_test
...
class Seq2SeqModel:
    ...
    def train_model(
        self, train_data1,best_accuracy,best_accuracy2, output_dir=None, show_running_loss=True, args=None, eval_data=None, verbose=True, **kwargs,
    ):
        """
        ...
        train_dataset1 = self.load_and_cache_examples(train_data1, verbose=verbose)


        os.makedirs(output_dir, exist_ok=True)

        global_step, tr_loss,best_accuracy2 = self.train(
            train_dataset1,
            output_dir,
            best_accuracy,best_accuracy2,
        ...
        return best_accuracy2

    def train(
        self, train_dataset,output_dir, best_accuracy1,best_accuracy2, show_running_loss=True, eval_data=None, verbose=True, **kwargs,
    ):
        """
        ...
        model.zero_grad()
        print("number of train epochssss", args.num_train_epochs)
        ...
        for current_epoch in train_iterator:
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                if args.fp16:
                    with amp.autocast():
                        ...
                        #now it is op.loss and op.logits
                ...
                if show_running_loss:
                    ...
                    batch_iterator.set_description(
                        f"Epochs {epoch_number}/{args.num_train_epochs}. Running Loss: {current_loss}"
                    )
            ...
            accuracy1, accuracy2,accuracy = predict_val(model, self.device)
            ...
            if accuracy2 > best_accuracy2:
                ...
                print('---test dataset----')
                test_acc1, test_acc2,test_acc = predict_test(model, self.device)
                with open('./MAMS_best_accuracy1.txt', 'a') as f0:
                    f0.writelines('batch: '+str(args.train_batch_size)+' accumulation_steps: '+str(args.gradient_accumulation_steps)+\
                                  ' lr: '+str(args.learning_rate)+' epochs: '+str(args.num_train_epochs)+' epoch: '+str(epoch_number)+' pol_val_accuracy: '+str(best_accuracy1)+\
                                  ' pol test_accuracy: '+str(test_acc1)+' intt val_accuracy: '+str(best_accuracy2)+\
                                  ' intt test_accuracy: '+str(test_acc2)+ 'combined acc: '+str(test_acc)+ '\n')
        ...
        return global_step, tr_loss / global_step, best_accuracy2

    def eval_model(self, eval_data, output_dir=None, verbose=True, silent=False, **kwargs):
        print("evalllllllllllllll modellllllllllllll")
        """
    ...
    def _get_inputs_dict(self, batch):
        ...
        if self.args.model_type in ["bart", "marian"]:
            ...
            inputs = {
                ...
                "labels": lm_labels.to(device),#lm_labels
            }
